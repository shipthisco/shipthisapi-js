...
const shipthisapi_js_1 = require("../dist/shipthisapi-js");
describe('shipthis api wrapper', () => {
    let shipthis;
    let doc;
    beforeAll(async () => {
        shipthis = new shipthisapi_js_1.ShipthisAPI({
    ...
    it('can fetch a collection of documents', async () => {
        ...
        const documents = (await shipthis.getCollection('sea_shipment', params))
            ?.data?.data?.items;
        expect(documents?.length).toBeGreaterThanOrEqual(1);
    });
    it('can delete a document from a collection', async () => {
        const deleted = (await shipthis.deleteCollectionItem('sea_shipment', doc?._id?.$oid))
            ?.data;
